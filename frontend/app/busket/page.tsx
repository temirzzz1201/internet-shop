...
const Busket = () => {
  ...
  const handleUpdateQuantity = async (
    ...
    if (userId) {
      const action = await dispatch(
        updateCartItem({ id: productId.toString(), quantity: newQuantity })
      );
  ...
  const handleIncrement = (index: number) => {
    ...
    if (stock === undefined) {
      console.error(
        `Запас для продукта с id ${product.product.id} не определен`
      );
      return;
    }
  ...
  const totalQuantity = busketProduct.reduce(
    ...
    <AppContainer title="Оформление заказа" myClass="justify-start">
      ...
      >
        {busketProduct.map((item, index) => (
          <Box
            ...
            p="10px"
          >
            <Box as="h3" mb="3" fontSize="md" fontWeight="semibold">
              Товар: {item.product?.name}
            </Box>
            <Box as="p" mb="2">
              Количество: {item.quantity}
            </Box>
            <Box as="p" mb="5" fontWeight="semibold">
      ...
      <Box display="flex" justifyContent="space-between" w="100%">
          ...
          {busketProduct.length > 0 ? (
            busketProduct.map((item, index) => (
              <Box
                ...
                p="10px"
              >
                <Box as="h3" mb="3" fontSize="xl" fontWeight="semibold">
                  Товар: {item.product?.name}
                </Box>
        ...
        <Box bg="orange.100" p="5" borderRadius="20px" maxH="fit-content">
          <Box as="h3" mb="3" fontSize="2xl" fontWeight="semibold">
          ...
          <Box>
            ...
            <Button colorScheme="teal" size="lg" onClick={handleOrder}>
              Оформить заказ
            </Button>
            <Button
              colorScheme="red"
