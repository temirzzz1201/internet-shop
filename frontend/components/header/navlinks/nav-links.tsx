...
const links = [
  ...
  { id: 4, title: 'Profile', path: '/profile', blueImgSrc: profileSrcBlue, whiteImgSrc: profileSrcWhite },
        ...
        links.map(link => (
          <li key={link.id} className="mr-5 flex items-center">
            {link.title === "Logout" ? (
              ...
              <Link href={link.path} passHref className={pathname === link.path ? "text-blue-300 underline flex items-center" : "text-white flex"}>
                {link.title}
                {link.title === "Profile" && (
                  <Image
                    className={pathname === link.path ? "h-6 w-6 pl-2" : "w-6 h-6 pl-2"}
                    src={pathname === link.path ? link.blueImgSrc : link.whiteImgSrc}
                    alt={link.title}
                  />
                )}
              </Link>
