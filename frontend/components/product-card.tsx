  ...
  return (
    <Box className={stock === 0 ? 'pointer-events-none' : ''}>
      <AppModal isOpen={isModalOpen} onClose={handleClose} title={product.category.name}>
        <EmblaCarousel slides={imageUrls} options={OPTIONS} autoPlayFlag imageClass='420' />
      </AppModal>

      <AppModal isOpen={isProductModalOpen} onClose={handleCloseProduct} title={product.category.name}>
        <Box mb='4'>
          <EmblaCarousel slides={imageUrls} options={OPTIONS} autoPlayFlag imageClass='420' />
        </Box>
        <Box mb='4'>
          <HStack>
            <Button size='sm' {...dec} isDisabled={stock === 0}>-</Button>
            <Input size='sm' {...input} textAlign='center' isDisabled={stock === 0} />
            <Button size='sm' {...inc} isDisabled={stock === 0}>+</Button>
          </HStack>
        </Box>
        <Box mb='5'>
      ...
      >
        ...
        <Text mb='3' fontSize='xs'> {formatDate(product.createdAt)} </Text>
      </Box>  
    </Box>
  );
}
